...
public class CellSelector : MonoBehaviour
{
    ...
    public Action<Vector2> OnStartPoint; //= delegate { };
    public Action<Vector2> OnEndPoint; //= delegate { };
    ...
    private CellView Raycast(Ray ray)
    {
        ...
        var resultCell = default(Cell);
        RaycastHit hit;
        if (Physics.Raycast(ray, out hit))
        {
            result = hit.transform.GetComponent<CellView>();
            resultCell = hit.transform.gameObject.GetComponent<Cell>();
        }
        return result;
    }
